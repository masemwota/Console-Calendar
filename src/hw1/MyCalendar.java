...
public class MyCalendar {
	...
	private SHORTDAYS[] arrayShortDays = SHORTDAYS.values();
	private SHORTMONTHS[] arrayShortMonths = SHORTMONTHS.values();
	...
	public void displayMonth()
	{
		...
		descriptor += arrayOfMonths[myCal.get(GregorianCalendar.MONTH)]; 
		descriptor += " ";
		descriptor += myCal.get(GregorianCalendar.YEAR); //year of this view
		descriptor += "\n";
		...
		GregorianCalendar temp = new GregorianCalendar(myCal.get(Calendar.YEAR), myCal.get(Calendar.MONTH), 1);
		...
		for(int j = 1; j < maxDays+firstDay; j++)
		{
			...
			temp.set(myCal.get(Calendar.YEAR), myCal.get(Calendar.MONTH), j - firstDay+1); 
